...
class HomeViewModel @Inject constructor(
    ...
    fun create(temp: ArrayList<StepUI>): ArrayList<StepUI> {
    ...
    private fun ArrayList<StepUI>.add(id: Int, day: Int, month: Int, year: Int) {
        add(
            StepUI(id + 1, name(day, month, year), day, month, year, 0, 0, 0.0, 0, 0.0, 0, false)
        )
    }
    ...
    fun sum(list:ArrayList<StepUI>,mod:Int): Int {
        var total = 0
        for (i in list) {
            total += when (mod) {
                1 -> i.step
